...
def render_page(rows: list[Mention] | None = None, error: str = "", form: dict[str, str] | None = None) -> bytes:
    ...
    table_rows = ""
    for r in rows:
        # Keep this tuple-concatenation fully closed to avoid SyntaxError.
        table_rows += (
            "<tr>"
    ...
    results_header = f"<p><strong>{len(rows)}</strong> result(s).</p>" if rows else "<p><strong>0</strong> result(s). Try another spelling, market, or timeout.</p>"
...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        try:
            ...
            self._send_html(render_page(error=f"Search failed: {exc}", form=view_form), status=500)
